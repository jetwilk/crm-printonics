...
const emptyForm = { fabricante: "", modelo: "", moeda: "EUR" };
...
export function MachinesPage() {
  ...
  const { data: machines = [], isLoading, error } = useQuery({
    queryKey,
    queryFn: () => machinesApi.list(showArquivadas),
  });
  ...
  const filtered = machines.filter(
    (m) =>
      m.fabricante.toLowerCase().includes(debouncedSearch.toLowerCase()) ||
      m.modelo.toLowerCase().includes(debouncedSearch.toLowerCase()) ||
      (m.pais ?? "").toLowerCase().includes(debouncedSearch.toLowerCase())
  );
  ...
  return (
    <div>
      ...
      {isLoading ? (
        ...
        <EmptyState message={showArquivadas ? "Nenhuma máquina arquivada." : "Nenhuma máquina encontrada."} />
      ) : (
          ...
          {filtered.map((machine) => (
            ...
            >
              ...
              <div className="space-y-1.5 text-sm text-gray-600 dark:text-gray-300">
                {machine.formato && (
                  <div className="flex items-center gap-2">
                    <span>📐</span><span>{machine.formato}</span>
                  </div>
                )}
                {machine.cores != null && (
                  <div className="flex items-center gap-2">
                    <span>🎨</span><span>{machine.cores} cores</span>
                  </div>
                )}
                {machine.pais && (
                  <div className="flex items-center gap-2">
                    <span>🌍</span><span>{machine.pais}</span>
                  </div>
                )}
